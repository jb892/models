...
def export_one_scan(scan_name, output_filename_prefix):
    ...
    logger.info('Num of instances: ', num_instances)

    bbox_mask = np.in1d(instance_bboxes[:, -1], OBJ_CLASS_IDS)
    instance_bboxes = instance_bboxes[bbox_mask, :]
    logger.info('Num of care instances: ', instance_bboxes.shape[0])
...
def batch_export():
    if not os.path.exists(OUTPUT_FOLDER):
        logger.info('Creating new data folder: {}'.format(OUTPUT_FOLDER))
        os.mkdir(OUTPUT_FOLDER)

    for scan_name in tqdm(TRAIN_SCAN_NAMES):
        logger.info('-' * 20 + 'begin')
        logger.info(datetime.datetime.now())
        logger.info(scan_name)
        ...
        try:
            ...
            logger.error('Failed export scan: %s' % (scan_name))
        logger.info('-' * 20 + 'done')
