...
def compute_vote_loss(end_points):
    ...
    seed_inds = end_points['seed_inds'].cast(dtype='int64')  # B,num_seed in [0,num_points-1]
    ...
    vote_loss = fluid.layers.reduce_sum(votes_dist * seed_gt_votes_mask.float()) / (fluid.layers.reduce_sum(seed_gt_votes_mask.float()) + 1e-6)
    return vote_loss
...
def compute_objectness_loss(end_points):
    ...
    criterion = fluid.layers.softmax_with_cross_entropy(torch.Tensor(OBJECTNESS_CLS_WEIGHTS), reduction='none')
...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    mean_size_arr_expanded = fluid.create_lod_tensor(mean_size_arr.astype(np.float32), place=fluid.).unsqueeze(0).unsqueeze(
...
def get_loss(end_points, config):
    ...
    end_points['pos_ratio'] = \
        fluid.layers.reduce_sum(objectness_label.float().cuda()) / float(total_num_proposal)
    end_points['neg_ratio'] = \
        fluid.layers.reduce_sum(objectness_mask.float()) / float(total_num_proposal) - end_points['pos_ratio']
    ...
    obj_acc = fluid.layers.reduce_sum((obj_pred_val == objectness_label.long()).float() * objectness_mask) / (
